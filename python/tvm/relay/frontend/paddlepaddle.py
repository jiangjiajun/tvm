...
def convert_multiclass_nms(g, op, block):
    ...
    try:
        boxes_shape = infer_shape(boxes)
        scores_shape = infer_shape(scores)
    except Exception:
        msg = "Only fixed shape for multiclass_nms3 is supported, try to use shape_dict in `from_paddle` "
        msg += "while you import model from PaddlePaddle."
        raise tvm.error.OpNotImplemented(msg)
    batch_size, num_boxes, num_cord = boxes_shape
    batch_size, num_classes, num_boxes = scores_shape
...
def convert_yolo_box(g, op, block):
    ...
    if clip_bbox:
        try:
            ...
            msg = "Only fixed shape for yolo_box is supported, try to use shape_dict in `from_paddle` "
            msg += "while you import model from PaddlePaddle."
            raise tvm.error.OpNotImplemented(msg)
        ...
        max_w_h = _op.reshape(new_img_size - _expr.const(1.0), (-1, 1, 2))
