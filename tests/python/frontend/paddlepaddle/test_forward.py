    ...
    with tvm.transform.PassContext(opt_level=3):
        for target, dev in tvm.testing.enabled_targets():
            ...
            for baseline_output, compiled_output in zip(baseline_outputs, tvm_output):
                ...
                print("===========", baseline_output.dtype, compiled_output.dtype)
...
def test_forward_look_up():
    ...
    @paddle.jit.to_static
    def look_up1(inputs, weight):
        return nn.functional.embedding(inputs, weight, padding_idx=1)
    ...
    input_data1 = paddle.to_tensor(np.array([1, 2, 1]).astype("int32"))
    verify_model(look_up1, input_data=[input_data, weight])
